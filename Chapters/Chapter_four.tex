...
Fig.~\ref{fig:pl_encp_annealing} shows the evolution of the PL spectra for a representative encapsulated chip (chip 2-10) across a series of annealing steps. Several distinct stages are observed.

\textbf{Stage I: Spectral stability (up to $\sim$700~K).} Below 700~K, no new spectral features emerge. The excitonic features identified in Section~\ref{subsec:Low-Temperature PL Spectrum}---the neutral exciton, trion doublet, negative biexciton, dark trion $T^I$, semidark trion $T'$, and phonon replicas---are present with unchanged relative intensities and linewidths. 
...
To enable a direct comparison of the spectral changes induced by annealing, the PL spectra after 400~K and 900~K annealing are replotted with the energy axis referenced to the neutral exciton position $X_A^0$. This representation eliminates the contribution of the rigid ensemble redshift and allows the intrinsic spectral evolution to be assessed independently. We observe that the exciton and trion peak are suppressed after 900K and the new peak $X_L'$ which is at 1.910~eV, corresponding to an energy offset $\Delta E = E(X_A^0) - E(X_L') \approx 120$~meV become dominant after 900K annealing. This offset is smaller than the $\sim$220~meV offset observed in non-encapsulated samples, possibly related to the well known decrease of binding energy in encapsulated samples due to enhanced dielectric screening \cite{raja-2017}.
...
The PLE spectrum displays a series of well-resolved resonances that can be directly assigned to intrinsic optical transitions of monolayer WS$_2$. The dominant resonances appear at approximately 2.025~eV ($X_A^0$: 1s) and 1.99~eV (trion $T$), along with their respective 2s excited states. This pattern is consistent with the PLE spectrum of the pristine encapsulated sample (Section~\ref{subsec:PLE Spectroscopy Exciton and Excited States}), confirming that $X_L'$ is activated by the same optical transitions as the intrinsic excitonic emission.
