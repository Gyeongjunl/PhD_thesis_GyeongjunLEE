...
The most remarkable property of $X_D$ is its spectral narrowness. Fig.~\ref{fig:linewidth_xd} shows PL spectra of the $X_D$ peak (chip 3-4, 1.945~eV) recorded with low excitation power at 0.1~$\mu$W, using the 1800~gr/mm grating of the $f = 300$~mm double spectrometer (instrumental spectral resolution $\approx 0.2$~meV). As shown in Fig.~\ref{fig:linewidth_xd}(a), $X_D$ is well isolated from the surrounding peaks concentrated in the 1.96--2.00~eV range, with a gap of $\gtrsim$15~meV to the nearest neighboring feature. This spectral separation is critical: it enables a narrow bandpass filter to select $X_D$ exclusively, avoiding spectral contamination from neighboring emission features in all subsequent optical characterization measurements, and most importantly in photon-correlation experiments.

Fig.~\ref{fig:linewidth_xd}(b) shows a zoomed PL spectrum of $X_D$. Lorentzian fitting yields a sub-meV FWHM of $\approx 0.218$~meV, close to the instrumental spectral resolution of the spectrometer, and more than an order of magnitude narrower than the $X_L$ peak recorded under identical conditions ($\sim$2--3~meV). The evolution of this linewidth as a function of excitation power will be examined in the following section.

This dramatic difference in linewidth indicates that $X_D$ represents a qualitatively different class of emission: a narrow, discrete emission line from a single quantum emitter (or a very small number thereof), rather than an inhomogeneous ensemble.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{Figure/Chapter_five/linewidth_xd.png}
    \caption{Spectral characterization of $X_D$ (chip 3-4, $\lambda_{\text{laser}} = 503$~nm pulsed supercontinuum, $P = 0.1~\mu$W, $T \approx 3.6$~K). (a) PL spectrum showing $X_D$ at 1.945~eV, spectrally isolated from the surrounding emission features concentrated in the 1.96--2.00~eV range. (b) Zoomed PL spectrum of $X_D$ with a Lorentzian fit (red curve), yielding a FWHM of $\approx 0.218$~meV.}
    \label{fig:linewidth_xd}
\end{figure}
