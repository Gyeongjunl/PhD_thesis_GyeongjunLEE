...
Having established the exciton emission energy as a temperature indicator, we applied this calibration to monitor the local temperature during in-situ annealing of hBN-encapsulated WS$_2$ monolayers.

In practice, the electrical current applied to the micro-heater is gradually increased while simultaneously recording the PL spectrum. The evolution of the neutral exciton ($X_A^0$) energy is monitored in real time. Fig.~\ref{fig:pl_evolution}(a) shows the evolution of PL with increasing temperature. Because of bandgap renormalization at higher temperature, the position of exciton emission is moving to lower energy (red-shift). By monitoring the position of the peak we can estimate temperature using Pässler's model (Fig.~\ref{fig:pl_evolution}(b)). The PL spectra remain well-defined up to temperatures of approximately 925~K. A continuous redshift of the exciton energy is observed as the heating current is increased, demonstrating that the exciton energy can be used to track the local temperature during in-situ annealing.

However, as shown in Fig.~\ref{fig:pl_evolution}(c), at elevated temperatures, the PL intensity decreases significantly, making it difficult to reliably extract the exciton energy. This sets an upper limit to the temperature range over which exciton-based thermometry can be applied. In this regime, alternative methods are required for temperature estimation.
