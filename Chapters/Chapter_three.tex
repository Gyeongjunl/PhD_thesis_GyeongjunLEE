...
We first applied this approach to non-encapsulated WS$_2$ monolayers over a broad annealing range. Fig.~\ref{fig:pl_evolution} (a) shows the evolution of PL with increasing temperature. Because of bandgap renormalization at higher temperature, the position of exciton emission is moving to lower energy (red-shift). By monitorning the position of the peak we can estimate temperature using Passler's model (Fig.~\ref{fig:pl_evolution}(b)). With this measurement, we obtained robust PL evolution using hBN-encapsulated WS$_2$ monolayer. The PL spectra remain well-defined up to temperatures of approximately 925K. A continuous redshift of the exciton energy is observed as the heating current is increased, demonstrating that the exciton energy can be used to track the local temperature during in-situ annealing. 

However, as shown in Fig.~\ref{fig:pl_evolution}(c), at elevated temperatures, the PL intensity decreases significantly, making it difficult to reliably extract the exciton energy. At higher temperatures, the PL intensity is strongly reduced. This sets an upper limit to the temperature range over which exciton-based thermometry can be applied. In this regime, alternative methods are required for temperature estimation.
...
\subsection{Complementary Temperature Estimation Using Black Body Radiation}

At elevated temperatures above approximately 900 K, the PL intensity decreases significantly, and blackbody radiation begins to dominate the optical signal. In this regime, the excitonic emission can no longer be reliably resolved. Instead, the thermal emission spectrum was used as an alternative temperature indicator. By analyzing the blackbody radiation, the temperature of the micro-heater was independently estimated, enabling calibration in the high-temperature regime.
Fig.~\ref{fig:blackbody} presents the blackbody radiation emitted by the membrane, which provides an extended temperature calibration at elevated temperatures. The blackbody spectra are fitted, after being corrected by the spectral response of the setup, using Planck's law:
...
At 925K (determined from the exciton position), the temperature estimated from the blackbody radiation fit yields 924+-3.5 K, in excellent agreement with the exciton-based calibration obtained fom Passler's model. For the remaining temperature points, temperatures can be precisely estimated.
\begin{figure}[htbp]
	\centering
